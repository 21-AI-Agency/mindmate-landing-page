...
**App Store / Google Play:** Görünüm aynı kalır; sadece `site.json` içinde `hero.badges` ve `downloadCta.badges` altındaki `href` değerlerini kendi mağaza linklerinizle değiştirin.

**Footer “Follow” başlığı:** `site.json` → `footer.socialTitle` alanını uygulama isminizle güncelleyin (örn. `"Follow MyApp"`).
